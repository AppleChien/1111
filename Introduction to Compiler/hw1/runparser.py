...
def parsetable(t_list,nt_list):
	levels= levels = (['action']*len(t_list) + ['goto']*len(nt_list))
	columns = t_list+nt_list
...
def run():
	...
	for i, j in table.items():
		print(str(i)+'     |     '+'     |     '.join(list(j.get(sym,' ') if type(j.get(sym))in (str , None) else next(iter(j.get(sym,' ')))  for sym in sym_list))+'         |')
		#print(i, "\t", j)
		s, r=0, 0
...
count=len(open('../input/2/2_testdatacopy.txt','r').readlines())
f = open('../input/2/2_testdatacopy.txt','r')
